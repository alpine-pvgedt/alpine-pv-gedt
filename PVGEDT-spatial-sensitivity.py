...
### FUNCTION TO PLOT ILLUSTRATIVE POWER CURVES FOR TWO SITES IN THE SPATIAL SENSITIVITY ANALYSIS ###

def get_power_sensitivity(site_latitude,site_longitude,site_altitude,topo_sunrise,topo_sunset):
    
    # create time series for desired day, with timestamps at 1-minute intervals
...
# set locational data for Site E
latitude_E = 46.8362
longitude_E = 9.813
altitude_E = 2442
...
# set locational data for Site H
latitude_H = 46.83928
longitude_H = 9.81114
altitude_H = 2526
...
# plot baseline and PV-GEDT power for Site E
...
# on the same curve, plot baseline and PV-GEDT power for Site H
...
# estimate the energy yields using trapezoidal integration
E_pvgedt_yield = np.trapezoid(E_power_pvgedt,dx=1/60) # dx is the x-spacing (1 min intervals = 1/60 hours)
H_pvgedt_yield = np.trapezoid(H_power_pvgedt,dx=1/60)

print(E_pvgedt_yield)
print(H_pvgedt_yield)
