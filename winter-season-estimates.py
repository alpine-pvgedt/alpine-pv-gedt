...
def estimate_monthly_yield(start_date,end_date):
   # monthly_dates = pd.Series(pd.date_range(start=pd.to_datetime(start_date, dayfirst = True).tz_localize('Europe/Zurich'),
                                 # end=pd.to_datetime(end_date, dayfirst = True).tz_localize('Europe/Zurich')))
...
################### GET MONTHLY POWER OUTPUT ESTIMATES #################
...
output_data.to_csv('Monthly_Yield_Estimates.csv', index=False)
