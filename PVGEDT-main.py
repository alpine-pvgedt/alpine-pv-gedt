...
# generative AI disclosure: function written/refactored with assistance from Claude Sonnet 4.6 via the ELM portal
# this largely took the form of bug fixes and inspiration - at no point was the entire script copied verbatim from gen AI output

def calculate_energy_yield(date: str,topo_loss_factor, plot_irrad):
    
    # get row index for the desired date from the input_data csv file 
    ...
    ### optional: plot irradiance from the PV-GEDT model against the measured data
    
    if plot_irrad == True:
        fig2,ax2 = plt.subplots()
        
        # get copies of the baseline dni and ghi values
        dni_pvgedt = dni.copy()
        ghi_pvgedt = ghi.copy()
        
        # modify these to account for the topographical sunrise and sunset
    ...
    ###### calculate energy yield for the measured data ###
   
    measured_power = measured_power.dropna() # remove rows with nan values of power
    power_measured = measured_power['Pmpp_W'] # extract pwower output column from dataframe
    ...
    # get the measured energy yield using trapezoidal integration
    ...
    # get baseline and PV-GEDT model yields over the measured time period
...
############### Function to call main function and arrange output ################
...
# must be set to zero if including irradiance plots

pvgedt_loss_factor = 0

# Error Analysis: plot irradiance plots? Set True/False as desired. If true,
# pvgedt_loss_factor must be set to zero!
...
# organise data and output to csv file
...
output_data.to_csv('Energy_Yield_Outputs.csv', index=False)
